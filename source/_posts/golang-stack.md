...
我们知道`CPU`有实模式和保护模式，系统刚刚启动的时候是运行在实模式下，然后经过一系列初始化工作以后，`Linux`会把`CPU`的实模式改为保护模式（具体就是修改`CPU`的`CR0寄存器`相关标记位），在保护模式下，`CPU`访问的地址都是虚拟地址(逻辑地址)。`Linux` 为了每个进程维护了一个单独的虚拟地址空间，虚拟地址空间又分为“`用户空间`”和“`内核空间`”。 虚拟地址空间更多相关可以看[Linux内核虚拟地址空间](https://www.jianshu.com/p/174c1da40c03)这篇文章。
