...
我们先看下[Demo1生成的汇编代码](https://godbolt.org/z/vrzfPz4rz)
	...
	main_pc189:
	        ...
	        CMPQ    ""..autotmp_6+72(SP), DX // 3 > DX (i < 3)
	        JGT     main_pc206  // 大于 3 跳转到 body 模块 , 执行 arr = append(arr, v)
	        JMP     main_pc358  // i>3 循环结束。执行后续打印代码。
	main_pc206:
	 ..........................
...
我们再来看下`golang`编译器是怎么对`for range`代码编译优化的。翻了下`Golang`编译器[源码](https://github.com/golang/go/blob/master/src/cmd/compile/internal/walk/range.go#L85)
...
可以看到 `ha := a` 这句代码，`for range`一个`Array`或者一个`Slice`的时候，会先`Copy`一下这个`Array`或者`Slice`。所以在循环的时候`Append`元素到`Slice`中去，并不会改变循环的次数。
...
这里，我们可以看到，`str`长度是`6`，因为`Golang`的源码默认都是用的`UTF-8`编码。

[UTF-8（8-bit Unicode Transformation Format）](https://zh.m.wikipedia.org/zh-hans/UTF-8)是一种针对Unicode的可变长度字元编码，也是一种前缀码。它可以用一至四个字节对Unicode字符集中的所有有效编码点进行编码
...
再看下`for`循环的的步长是如何算的，汇编代码如下。
	...
	main_pc172:
	        ...
	        // 把`decoderune`函数第二个返回值存到了 ""..autotmp_5+64(SP) 中，上面会把这个赋值给DX，DX再去跟6比较
...
我们可以知道，这个函数会返回当前字符串`k`之后的`rune`字符和`rune`字符对应的位置。所以`demo`的循环`idx`是`0`和`3`，因为`0`、`3`分别是两个字符的其实位置。
