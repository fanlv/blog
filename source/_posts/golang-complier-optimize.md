...
还有一些场景，激进的死代码删除算法会删除没有输出的无线循环，从而改变程序的含义。因为在原来的程序不产生任何输出的情况下，删除这种无限循环后，程序会执行该循环之后的语句，而这些语句有可能产生输出。在许多环境下，这被认为是不可接受的（详见[《现代编译原理》- 虎书](https://book.douban.com/subject/1806974/)`19.5`章节）。
...
* 临时变量，在表达式求值期间和其他较短活跃期内使用和计算出的临时值。
