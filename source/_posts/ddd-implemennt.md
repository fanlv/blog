...
我们`message`服务经过`4`年的迭代，项目的可维护性、代码的可读性、功能的可扩展性越来越差，经常改一个小功能牵一发而动全身，新来的同学对`message`服务不能快速上手，而且因为代码可读性不高，导致新同学修代码经常会出现`bug`。还有一部分原因是大多数人都是第一次用`Golang`，对项目的分层也没有一个统一的规范，导致最后成了一个四不像的框架。
...
4. 产出一个最佳实践规范，作为整个`Messenger`项目指导规范。
...
1. 权限校验、参数校验。
2. 发送或订阅领域事件等。
3. `Domain Obejct` 组装（简单接口可以不用`DO`）
